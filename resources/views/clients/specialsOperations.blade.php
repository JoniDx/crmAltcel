...
//cambio product
	$w5finish.on('click', function( ev ) {
        ...
        let stripe_id = $('#ratesChangeProduct option:selected').attr('data-stripe');
		...
		if(validated){
            ...
            $.ajax({
                ...
                data: data,
                success: function(response){
                    if(type == 'internalExternalPaymentChange' || channel == 5){
                        if(channel == 1){
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            offersAndRates.forEach(function(element){
                options+="<option value='"+element.rate_id+"' data-rate-id='"+response.dataMSISDN.rate_id+"' data-stripe='"+element.stripe_id_product+"' data-rate-price='"+element.rate_price+"' data-offerID='"+element.offerID+"' data-offer-id='"+element.offer_id+"' >"+element.rate_name+"</option>"
            })
...
//compra
	$w6finish.on('click', function( ev ) {
        ...
        let stripe_id = $('#ratesPurchaseProduct option:selected').attr('data-stripe');
        let token = $('meta[name="csrf-token"]').attr('content');
        ...
        if($('input[name="purchaseProductRadio"]').is(':checked')) {
            if(validated){
                ...
                if(radioOption == 'purchaseProductFree' || radioOption == 'purchaseProductCollect'){
                    data = {msisdn:msisdn, offer:offerID, user_id:user_id, stripe_id:stripe_id, rate_id:rate, offer_id:offer_id, price:price, comment:comment, reason:reason, status:status};
                    route = "{{route('purchase')}}";
                    method = 'GET';
                }else if(radioOption == 'purchaseProductPayment'){
                    data = {
                        _token:token, number_id: number_id, name: name, lastname: lastname, email: email,
                        cel_destiny_reference: cel_destiny_reference, stripe_id:stripe_id, amount: amount, offer_id: offer_id,
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            packsSurplus.forEach(function(element){
                ...
                options+="<option value='"+element.offerID+"' data-rate-price='"+element.price_sale+"' data-stripe='"+element.stripe_id_product+"' data-rate-id='"+dataMSISDN.rate_id+"' data-offer-id='"+element.id+"'>"+element.name+"</option>"
            })
