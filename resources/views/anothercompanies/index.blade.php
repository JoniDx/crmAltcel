...
<td>{{$prospect->tipo}}</td>
                    ...
                    <td>
                        <button class="btn btn-success btn-sm attend" data-name="{{$prospect->name}}"  data-telefono="{{$prospect->telefono}}"  data-idprospect="{{$prospect->id}}" data-cp="{{$prospect->cp}}" data-tipo="{{$prospect->tipo}}"><i class="fa fa-check"></i></button>
                    </td>
                </tr>
                @endforeach
            </tbody>
                ...
                <form class="col-md-12 d-none" id="formClient" action="{{route('clients.storeFromAnotherCompany')}}" method="POST">
                    ...
                    <div class="col-md-4 mt-xs mb-md">
                        ...
                        <select id="interests" name="interests" class="form-control form-control-sm" required="">
                            ...
                            <!-- <option value="DISTRIBUIDOR" id="distribuidorOption" class="d-none">DISTRIBUIDOR</option> -->
                        </select>
                    </div>
...
<script >
    ...
    $('.attend').click(function(){
        ...
        if(dataTipo == 'distribuidor'){
            // $('#interests').find('[value="DISTRIBUIDOR"]').add();
            $("#interests").prepend("<option value='DISTRIBUIDOR' id='distribuidorOption'>DISTRIBUIDOR</option>");
            $('#distribuidorOption').removeClass('d-none');
            const select = document.querySelector('#interests');
            select.options[0].setAttribute('selected',true);

            
        }if(dataTipo == 'cliente'){
            // $('#interests').find('select').find('option[value]').remove();
            $('#interests').find('[value="DISTRIBUIDOR"]').remove();

            $('#distribuidorOption').addClass('d-none');
        }
    ...
    $('#saveProspect').click(function(){
        ...
        if($('#client_flag').is(':checked')){
            if(direccion != '' && apellido != ''){
                ...
                icon: 'warning',
                title: 'Woops!!',
                text:'El campo Apellido y Dirección no deben ir vacíos.'
            });
            }
        }else{
        }
